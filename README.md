...
✅ **多平台一键覆盖** - 内置 20+ 个主流广告平台 SDK，单次构建同时输出所有渠道素材，无需为每个平台重复开发
   ...
   - 以及**通用 MRAID** 构建：一份产物即可投放所有遵循 MRAID 规范的长尾平台（如 Aarki、Adikteev、BigaBid、YouAppi、Kayzen 等）
...
[立即获取 Bingo 应用](https://store.cocos.com/app/detail/8893) - 前往 Cocos 商店购买和下载

> 💡 **版本更新说明**：由于 Cocos 商店审核周期较长，商店版本可能会有一定滞后。如果您发现商店版本不是最新版本，可以直接购买后凭支付记录截图，通过邮箱（458264325@qq.com）或微信联系我们获取与体验版相同的最新版本。

## 前置准备：SDK 接入

在使用 Bingo 构建广告前，需要先在 Cocos Creator 项目中接入 [PlayableSDK.d.ts](./PlayableSDK.d.ts)。**这一步需要在游戏开发阶段完成**，接入 SDK 后广告才具备跳转商店、响应平台音频/暂停控制等能力。
...
2. 选择菜单「项目」→「构建发布」
3. 目标平台选择「Web Mobile」
4. ⚠️ **重要**：确保不勾选「MD5 Cache」选项（详见[打包配置注意事项](#打包配置注意事项)）
5. 点击「构建」按钮，等待构建完成
...
2. 添加或选择您的产品
3. 配置产品的 Apple URL 和 Google Play URL（用于广告跳转和平台配置）
...
> 💡 **提示**：部分渠道（如 Facebook、Google、TikTok、Unity Playworks）会输出 ZIP 包，其他渠道输出单个 HTML 文件。
...
这确保了压缩操作始终是有益的，不会因为压缩导致包体变大。此外，Bingo 采用 **Base122 编码**内嵌游戏资源，相比 Base64 数据膨胀率从约 33% 降至约 14%，进一步压缩最终文件体积。
...
调用栈底部的 `inject.js` 是**浏览器扩展注入的脚本**，并非可玩文件自身的 bug。某些扩展（如性能监控、广告分析工具等）会向页面注入节点，导致 Cocos 引擎报错。**该错误不影响游戏实际运行**，真实用户在 Facebook/Instagram 等平台的 WebView 中不存在浏览器扩展，不会遇到此问题。
...
> 由于 Cocos 商店审核周期较长，商店版本可能滞后于体验版。如需最新版本，可参照[获取 Bingo](#获取-bingo) 联系我们。

### 2.0.4

- **新增「通用 MRAID」渠道**：一份产物即可投放所有遵循 MRAID 规范的长尾平台（如 Aarki、Adikteev、BigaBid、YouAppi、Kayzen 等），无需为每个平台单独构建
- **新增「Unity Playworks（Luna）」渠道**：自动导出 `source.html`、`luna.json`、`playground.json` 组成的 Playworks ZIP 包，支持 `startGame` 启动协议、CTA 和 GameEnded API
- **新增渠道**：InMobi、快手（Kuaishou）、Kwai、MyTarget、巨量引擎（OceanEngine）、Pangle、Snapchat、Vungle / Liftoff
- **优化**：统一 AppLovin、Chartboost、InMobi 等 MRAID 渠道的生命周期表现（后台暂停/恢复、平台静音、横竖屏自适应），与 Unity 渠道行为一致
- **优化**：产品配置中的 Apple 链接统一命名为 **Apple URL**，并用于 Unity Playworks 的 `iosLink` 生成
- **修复**：解决使用 Spine 动画时在 Unity / AppLovin 测试 App 上卡在开场加载（splash loading）或黑屏的问题
- **修复**：InMobi 跳转商店逻辑；更新腾讯优量汇 SDK 地址
- **下线**：AdColony（已并入 Digital Turbine 生态，不再单独提供）

### 2.0.3

- 新增 Unity 渠道 MRAID 支持，增强 SDK 健壮性
- 优化 `mraid.open` 跳转与应用商店 URL 选择逻辑
- 完善 SDK 接入指南与产品介绍文档

### 2.0.2

- 早期版本，奠定多渠道一键构建、图片压缩与 Base122 内嵌等核心能力
